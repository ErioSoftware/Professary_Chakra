import { MobileThemeToggleButton, ThemeToggleButton } from './themeToggleButton'
...
export const Header: FunctionComponent = () => {
  const auth = useAuth()
	...
	return (
		<header className='flex items-center justify-between h-24 px-6 mx-auto max-w-7xl bg-greyscale-off-white dark:bg-greyscale-ash'>
			{/* eslint-disable-next-line @next/next/no-html-link-for-pages */}
			<Link href='/'>
				<a className='items-center desktop:inline-flex'>
					<svg
						width='49'
						height='48'
						viewBox='0 0 49 48'
					...
					<p className='hidden ml-3 desktop:inline text-desktop-link-large text-primary-default dark:text-primary-darkMode'>
						Professary
					</p>
				</a>
			</Link>
			<div>
				...
				<div className='items-center hidden space-x-8 desktop:flex'>
					<Link href='/'>
						<a
							className={`font-semibold text-desktop-link-x-small ${
								router.pathname === "/" ? "text-primary-default dark:text-primary-darkMode" : " text-greyscale-label"
							}`}>
							Home
						</a>
					</Link>
					...
					<div className='flex space-x-4'>
						{auth?.instance ? (
							<button
								onClick={() => auth?.logout()}
								type='button'
								className='px-6 py-2 font-semibold border-2 rounded-md border-greyscale-line dark:border-greyscale-label text-desktop-link-x-small text-primary-default dark:text-primary-darkMode'>
								Logout
							</button>
						) : (
							<>
								<Link href={"/login"}>
									<a className='px-6 py-2 font-semibold border-2 rounded-md border-greyscale-line dark:border-greyscale-label text-desktop-link-x-small text-primary-default dark:text-primary-darkMode'>
										Login
									</a>
								</Link>
								<Link href={"/register"}>
									<a className='inline-flex items-center px-6 py-2 font-semibold border-2 rounded-md border-primary-default dark:border-primary-darkMode bg-primary-default dark:bg-primary-darkMode text-desktop-link-x-small text-greyscale-bg dark:text-greyscale-ash'>
										Register
										<svg
											width='16'
											height='16'
											...
											xmlns='http://www.w3.org/2000/svg'>
											<path
												d='M6.66667 3.33337L11.3088 7.97549L6.66667 12.6176'
												stroke='currentColor'
												strokeWidth='2'
												strokeLinecap='round'
